      ...
      <div className="mx-auto w-full max-w-lg">
        <div className="mb-6 text-center">
          <Link to="/" className="inline-block" aria-label="AQAL home">
            <AqalLogo plate plateClassName="mx-auto h-16 w-16 rounded-[16px] p-2 sm:h-[4.5rem] sm:w-[4.5rem]" />
          </Link>
        </div>
        <div className="rounded-[2rem] border border-app-border bg-white p-6 shadow-soft sm:p-9">
          <p className="text-center text-xs font-bold uppercase tracking-[0.2em] text-brand-600">{eyebrow}</p>
          <h1 className="mt-3 text-center font-display text-3xl font-semibold text-app-text sm:text-4xl">{title}</h1>
          <p className="mx-auto mt-3 max-w-sm text-center text-sm leading-6 text-app-muted">{description}</p>
          {children}
        </div>
      </div>
    </section>
